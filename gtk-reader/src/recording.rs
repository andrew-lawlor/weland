//! Native microphone capture and Ogg Opus encoding for voice notes — ported
//! near-verbatim from `reader/src-tauri/src/recording.rs`. The capture/
//! resample/encode logic was already GTK-agnostic there (`cpal` + plain
//! `std::sync::mpsc`, no Tauri types touched); the only real change is
//! swapping Tauri's `State<AppState>`-based commands for plain functions —
//! GTK's single main-thread model doesn't need the `Arc<Mutex<..>>` Tauri's
//! multi-threaded command dispatch required.
...
// safely cross threads), so it can never live in shared, cross-thread state
...
pub fn start_voice_recording() -> Result<RecordingSession, String> {
...
/// Stops the session and returns the encoded Ogg Opus bytes.
pub fn stop_voice_recording(session: RecordingSession) -> Result<Vec<u8>, String> {
    ...
    ogg_opus::encode::<48000, 1>(&resampled).map_err(|e| format!("Failed to encode voice note: {e}"))
}
